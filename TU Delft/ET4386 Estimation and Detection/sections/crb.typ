...
除非该 PDF 的非零定义域取决于 $theta$（#Cre("TODO")）。
...
#blockquote([
    *关于得分函数和正则条件的直观含义*：
    ...
    正则条件的字面意思看起来就是：对任意参数值 $theta$ 来说，得分函数在随机观测 $bold(x)$ 下的期望都是 $0$，也就是在说观测整体的得分函数平均下来应该是 $0$，不会和给定的 $theta$ 自相矛盾。
...
#blockquote([
    *关于 Fisher 信息的直观含义*：
    ...
    再次注意，这里 Fisher 信息的这些等价定义*依赖于正则条件的成立*。正则条件表明得分函数的期望为 $0$，…… #Cre("TODO")
...
$
bold(w) ~ cal(N)(bold(0), bold(C)_w) quad <=> quad p(bold(w)) = 1/((2 pi)^(N/2) det(bold(C)_w)^(1/2)) exp[-1/2 bold(w)^T bold(C)_w^(-1) bold(w)]
$

由此定义高斯模型：

$
bold(x) = bold(h)(theta) + bold(w), quad bold(x) ~ cal(N)(bold(h(theta)), bold(C)_w)
$

或者写成概率密度函数为：

$
p(bold(x)) = 1/((2 pi)^(N/2) det(bold(C)_w)^(1/2)) exp[-1/2 (bold(x) - bold(h)(theta))^T bold(C)_w^(-1) (bold(x) - bold(h)(theta))]
$

有了分布函数，取对数并对参数求导得到得分函数：

$
s(bold(x);theta) = (partial ln p(bold(x); theta))/(partial theta) = (partial bold(h)^T (theta))/(partial theta) bold(C)_w^(-1) (bold(x) - bold(h)(theta))
$

以及二阶导：

$
(partial^2 ln p(bold(x); theta))/(partial theta^2) = (partial^2 bold(h)^T (theta))/(partial theta^2) bold(C)_w^(-1) (bold(x) - bold(h)(theta)) - (partial bold(h)^T (theta))/(partial theta) bold(C)_w^(-1) (partial bold(h) (theta))/(partial theta)
$

从而求得 Fisher 信息：

$
I(theta) = -EE[(partial^2 ln p(bold(x); theta))/(partial theta^2)] = (partial bold(h)^T (theta))/(partial theta) bold(C)_w^(-1) (partial bold(h) (theta))/(partial theta)
$
...
$
bold(x) = bold(h) theta + bold(w), quad bold(w) ~ cal(N)(bold(0), bold(C)_w)
$

代入得到 CRLB：

$
"var"(hat(theta)) >= 1 / (bold(h)^T bold(C)_w^(-1) bold(h))
$

同时参考 @sec:ed_crb_find_the_mvu_estimator，将线性高斯模型的得分函数整理为如下形式：

$
s(bold(x);theta) = (partial ln p(bold(x); theta))/(partial theta) &= bold(h)^T bold(C)_w^(-1) (bold(x) - bold(h) theta) \
&= underbrace(bold(h)^T bold(C)_w^(-1) bold(h), I(theta)) [underbrace((bold(h)^T bold(C)_w^(-1) bold(h))^(-1) bold(h)^T bold(C)_w^(-1) bold(x), g(bold(x))) - theta]
$
