...
#blockquote([
    *有关 “模型” 概念*：
    ...
    前面的问题分类中，对于笼统的 "模型" 概念，都将其具象化为了*状态空间模型*（state-space model），而且还是*离散*的情况。
    ...
    实际问题中也可能出现其他情况，例如扩展 Kalman 滤波可以一定程度上处理非线性模型的滤波问题。
])
...
主要介绍两类独立的线性系统辨识方法，一是*预测误差法*（prediction error methods），二是*子空间法*（subspace methods）。如前面注释中提到的，可以有各种不同的模型假设，这里*前者*常采用传递函数模型：
...
*后者*则采用状态空间模型（或者系统的输入输出行为可以用一个这样的有限维 LTI 状态空间模型等效表示），避开非线性优化问题，通过线性代数的手段求解系统辨识问题。
