...
#underline[接下来正式规定 SLS 问题]：给定关于随机变量 $theta$ 的先验 $theta ~ cal(N)(mu_theta, P_theta)$，$P_theta succ.curly.eq 0$，以及观察模型：
...
#pagebreak()
=== The Recursive Least-Squares (RLS) Algorithm <sec:lse_sls_rls>
...
#blockquote([
    *参数估计与状态估计、RLS 算法与 SLS 闭式解的关系以及遗忘因子的引入*：
    ...
    理论上，我们应该去修改这条状态转移方程，对状态的变化建模。我们意识到，这么做#underline[实际上就直接把问题变成了 Kalman 滤波问题]。但如果仅从 RLS 的角度出发，这条状态转移方程本来是隐含的，只是被我们显式写了出来。#underline[我们无法在不变成 Kalman 滤波器的前提下对 RLS 算法本身打某种补丁来修改这条隐含的方程]，这条 “静态参数假设” 是 RLS 推导乃至 SLS 问题的基础。我们能做的只有间接地实现这一点：#underline[既然无法改变状态模型，那么就改变 “观测数据如何影响我们的估计”，即改变第二行的观测方程]。
    
    #underline[从工程直觉的角度来说]，我们的解决方案是引入一个遗忘因子 $lambda$，把它作为惩罚施加在过去的数据上，即认为 “离现在越远的数据越不重要”，从而实现对参数变化的适应性（旧参数产生的旧观测数据随时间被遗忘，新参数产生的新观测数据占据估计主导）。而这一操作#underline[从公式上来说]就是让 $F_k$ 和 $W_k$ 随时间发生变化了，如前所述地改变了观测模型，从而间接影响了估计过程。
    
    #underline[模型本身就是输入到输出的映射，里面的状态转移和观测这两个步骤其实是我们在状态空间模型中人为划分的，为了更好的可解释性]。改变任意一条都可以实现改变整个模型的目的，只是状态的物理意义可能会被扭曲。
    
    什么叫 “物理意义” 呢？就是我们通过一些物理的事实定义了一个模型，这个状态可能有实际的含义，例如温度、坐标等。在这里，由于观测模型理论上是不应该改变的（SLS 问题规定好的），如果我们去改变了它，令其偏离了事实，那么其实就扭曲了模型包括其状态的物理意义，模型从此时起将变成一个 “黑箱”，管用但不再具备良好的可解释性。#underline[同时，通过调节一个简单的遗忘因子去代替设计状态转移模型，必然存在自由度不足的问题，使得没法实现精细的状态（参数）建模，应用场景有限]。
    
    总结来说，带遗忘因子的 RLS 算法更像是一种工程上的技巧，对原本算法打的一个补丁。这算是历史因素，RLS 最初不针对状态估计问题，遗忘因子的引入则是对数据应用 “加窗” 思想的一次自然的推广。
])
