...
#blockquote([
    *关于概率、条件概率和概率分布的符号约定*：
    ...
    对于一个随机变量 $X$，我们可以用 $X ~ p(x)$ 表示其服从分布函数 $p(x) := PP(X = x)$，其中的 $x$ 只是表示自变量的任意符号，所以有时也写成 $X ~ p(dot)$ 避免混淆，用点号表示该函数的自变量。在 MDP 中我们定义了形如 $S_(t+1) ~ P(dot mid(|) s_t, a_t)$、$pi (dot mid(|) s_t)$ 的分布函数，里面存在用于表示条件竖线符号，但#underline[实际上这是容易混淆的]。#underline[这些函数中的 "条件" 实际上表示 "参数"]，而参数按惯例最好用分号隔开，或者标记到上下标等地方。例如 $S_(t+1)$ 服从的分布实际上是 $p(s) := P(s; s_t, a_t)$，只有 $s$ 是自变量；$pi (dot mid(|) s)$ 实际最好写为 $pi (dot; s)$ 等。
...
总而言之，MDP 加上策略 $pi$ 这#underline[两个要素]构成了马尔可夫链，如 @fig:markov_chain_flow 所示。因此，随机轨迹序列 $Tau$ 的分布参数依赖 MDP 参数和 $pi$，可记为：
...
$
EE_pi [dot] = EE_(Tau~p_Tau^pi (dot)) [dot]
$ <equ:ee_pi_def>
...
$
V^pi (s) := EE_pi [sum_(t=0)^infinity gamma^t R_t mid(|) S_0 = s]
$ <equ:vfunc_def>
...
$
Q^pi (s, a) := EE_pi [sum_(t=0)^infinity gamma^t R_t mid(|) S_0 = s, A_0 = a]
$ <equ:qfunc_def>
...
#blockquote([
  *关于 “动作值函数-值函数” 递推关系的证明*：
  ...
  $
  Q^pi (s, a) &=^"def" EE_pi [sum_(t=0)^infinity gamma^t R_t mid(|) S_0 = s, A_0 = a] \
  &=^((1)) EE_pi [R_0 + gamma sum_(t=0)^infinity gamma^t R_(t+1) mid(|) S_0 = s, A_0 = a] \
  &=^((2)) r(s, a) + gamma EE_pi [sum_(t=0)^infinity gamma^t R_(t+1) mid(|) S_0 = s, A_0 = a]
  $

  式中第 $(1)$ 步拆出第一项 $R_0$ 和系数 $gamma$ 并调整索引（$t <- t+1$），第 $(2)$ 步代入 $R_0 = r(S_0, A_0)$ 并将其与系数 $gamma$ 从期望中提出来。表达式应用全概率公式可得：

  $
  r(s, a) + gamma sum_(s') P(s' mid(|) s, a) EE_pi [sum_(t=0)^infinity gamma^t R_(t+1) mid(|) S_0 = s, A_0 = a, S_1 = s']
  $

  由马尔可夫性，给定 $S_1 = s'$ 条件的情况下未来与 $(s_0, a_0)$ 无关，故消去相应条件得：

  $
  Q^pi (s, a) = r(s, a) + gamma sum_(s') P(s' mid(|) s, a) EE_pi [sum_(t=0)^infinity gamma^t R_(t+1) mid(|) S_1 = s']
  $
...
#blockquote([
  *关于贝尔曼期望公式的证明*：
  ...
  $
  V^pi (s) &=^"def" EE_pi [sum_(t=0)^infinity gamma^t R_t mid(|) S_0 = s] \

  &=^((1)) EE_pi [R_0 + gamma sum_(t=0)^infinity gamma^t R_(t+1) mid(|) S_0 = s] \
  ...
  第 $(2)$ 步是应用迭代期望公式（tower property of conditional expectation）将里面套上条件期望，引入额外条件 $A_0$ 和 $S_1$。这里两个变量来自外层 $EE_pi$，对内层可视为已知量，和内层 $EE_pi$ 的变量名重复了所以直接写符号而不是 $A_0 = A_0$ 等。

  第 $(3)$ 步内层 $EE_pi$ 利用马尔可夫性，已知 $t=1$ 时状态为 $S_1$，则条件分布和之前时刻的 $S_0 = s$、$A_0$ 都无关，从而省去条件。第 $(4)$ 步直接利用 @equ:vfunc_time_shift_prop 中的时移性质。

  第 $(5)$ 步是将轨迹分布 $p_Tau^pi (dot)$ 对 $(A_0, S_1)$ 的边缘分布取出来。具体地，轨迹分布是链上所有随机变量的联合分布，而第 $(4)$ 步所求期望仅依赖 $S_0$、$A_0$、$R_0$ 和 $S_1$，故无需考虑之后的 $A_1$ 等；余下的 $S_0 = s$ 和 $R_0 = r(S_0, A_1)$ 由条件和定义消去，只留下对 $(A_0, S_1)$ 的边缘分布。最后第 $(6)$ 步只是符号重命名（$S_1 -> S'$，$A_0 -> A$），得到更通用的形式。
...
其中两个 $*$ 式的含义仍表示需要 @equ:optimal_policy_exists_assumption 的最优策略存在；前者是为了将 $max_pi$ 放进期望中，后者是证明最优动作值函数贝尔曼方程时用到了该假设；最后一步依旧符号重命名。
