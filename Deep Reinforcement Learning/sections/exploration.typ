...
既然客观的偶然不确定性无法降低（irreducible），那么就让智能体更主观地评估 "不确定性"，#underline[优先选择还没有尝试过的动作]，就不会陷入循环。动作是否已经尝试过完全是从智能体的视角来说的，未尝试过的动作表示其对应状态空间尚未被完全探索，这种不确定性可称为*认知不确定性*（aleatoric uncertainty），不会像偶然不确定那样因系统性质导致无法降低。但这类策略也有问题，一是难以处理连续动作空间的情况，二是会导致过度探索——本来智能的策略就是希望用有限的算力探索过于庞大的状态空间中的低维有效信息，现在这样就有退回到暴力搜索的意思了。
